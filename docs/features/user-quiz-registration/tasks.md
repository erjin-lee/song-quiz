...
> **[`YOUTUBE_LINK_VERIFICATION_ENABLED`](../../../apps/api/src/quiz/quiz.constants.ts) = `false`(2026-09-03, 임시 조치).** 여러 유저가 동시에 퀴즈를 등록/수정하면 유튜브 스크래핑(영상 정보 조회·자동 검색) 요청이 짧은 시간에 몰려 유튜브 쪽에서 차단될 위험이 있어, 실제 검증을 잠시 꺼뒀다. 켜져 있는 동안은 링크 형식만 확인하고 항상 통과시키며, "자동으로 찾기"는 항상 실패로 응답한다. 앞으로 등록 신청을 큐에 넣고 하나씩 순차 처리하는 방식으로 바뀌면 다시 켤 계획이다 — **그 큐는 아직 설계도, 구현도 하지 않았다.** 재활성화하려면 이 상수만 `true`로 바꾸면 된다.
...
- 코드 리뷰를 거치며 유닛 테스트를 계속 보강했다(신규 파일: `link-verification-token.util.spec.ts`, `user-song.service.spec.ts` 포함). `yarn workspace api test`(36 suites, 344건) / `build` / `lint` 전체 통과 확인.
- **유튜브 링크 검증 임시 비활성화**(2026-09-03, 문서 최상단 콜아웃 참고): 동시 등록으로 인한 유튜브 차단 위험 때문에 `YOUTUBE_LINK_VERIFICATION_ENABLED` 상수(`quiz.constants.ts`)를 껐다. `YoutubeLinkValidationService.validate()`는 형식(videoId 파싱)만 확인하고 실제 스크래핑·제목 대조 없이 항상 통과시키며, `UserSongService.autoFillYoutubeLink()`(자동 찾기)는 검색을 시도하지 않고 항상 "찾지 못함"으로 응답한다. **등록 자체가 막히는 건 아니다** — `.../youtube-link/validate`를 거쳐야 하는 토큰 필수화 규칙(위 "즉시 검증을 우회한 등록 차단 + 토큰 필수화" 참고)은 그대로 유지되므로, 유저는 형식만 맞는 링크를 입력하고 "확인"을 눌러야 등록할 수 있다 — 다만 그 확인이 실제로 영상이 존재하는지·제목이 맞는지는 더 이상 보증하지 않는다. `youtube-link-validation.service.spec.ts`/`user-song.service.spec.ts`에 켜진 상태(기존 로직 검증용)/꺼진 상태 양쪽 테스트를 모두 남겨뒀다 — 재활성화 시 회귀를 바로 확인할 수 있다.
...
- **검증 토큰이 진짜로 필요해진 이유(빌더 UX에 직결)**: 백엔드가 이미 "모든 곡은 최근에 검증받은 토큰이 있어야 등록/수정 가능"으로 강제하기 때문에, 프런트도 곡을 담을 때마다 링크 편집 모달 → "확인" → 서버 검증 → ✅ 상태가 되는 흐름을 반드시 거치게 만들었다. **수정 화면 프리필(`GET /quizzes/:quizId`)은 저장된 링크를 다시 스크래핑해서 검증하지 않는다** — 처음엔(2026-09-03 1차 개선) 조회할 때마다 유튜브를 다시 긁어서 재검증했는데, 곡 수만큼 외부 요청이 무제한으로 동시에 나가는 문제가 지적됐다(코드 리뷰). 그래서 최종적으로 "이 QuizSong 행이 지금 존재한다는 사실 자체가 이미 검증을 통과했다는 증거"로 취급하도록 바꿨다 — 안전망(`runBackgroundSafetyNet`)이 재검증에 실패한 곡은 행 자체를 삭제하므로, 살아남은 행은 항상 검증을 통과한 상태만 나타낸다. 덕분에 수정 화면을 열 때 외부 요청이 전혀 나가지 않고, 모든 곡이 즉시 확인 완료 상태로 시작한다.
...
- **코드 리뷰 6차(2026-09-03)에서 고친 것**: `GET /quizzes/:quizId`가 곡 수만큼 유튜브 검증을 동시에(`Promise.all`) 실행해서, 곡이 많은 퀴즈를 열 때마다 외부 요청이 무제한으로 몰릴 수 있다는 지적을 받았다. "저장된 링크를 다시 검증하지 않고 그대로 신뢰"하는 방향으로 설계를 바꿔 해결했다(위 "검증 토큰이 진짜로 필요해진 이유" 항목 참고) — 재검증 호출 자체가 없어지므로 동시 요청 문제도 함께 사라진다. 같은 라운드에서 지적된 나머지 2건(정답 후보 응답 경쟁, 만료된 로컬 초안이 확인 완료로 복구됨)은 우선순위상 지금은 고치지 않기로 결정 — 아래 "아직 안 고친 것"에 추가.
- **코드 리뷰 7차(2026-09-03)에서 고친 것**: 위 6차 변경이 새 버그를 하나 만들었다 — 수정 화면 프리필(`GET /quizzes/:quizId`)이 기존 곡에 항상 MANUAL 토큰을 발급하는데, `PATCH /quizzes/:quizId`는 기존 곡을 전부 지우고 새로 만드는 방식이라 이 MANUAL 토큰이 그대로 안전망(`runBackgroundSafetyNet`)의 제목 매칭 여부를 결정해버렸다. 그 결과 원래 AUTO로 등록돼 제목 매칭 예외가 필요했던 곡은, **설명만 고쳐서 저장해도** 수정 안전망이 처음으로 제목 매칭을 시도해서 삭제될 수 있었다(코드 리뷰 지적). `updateQuiz`가 삭제 전에 songId별 기존 `youtubeVideoId`를 기억해뒀다가, 제출된 videoId와 같으면("링크를 안 건드림") 토큰 출처와 무관하게 안전망 제목 매칭을 생략하도록 수정 — "예전에 통과했음을 신뢰한다"는 원칙을 상세 조회뿐 아니라 수정 안전망에도 동일하게 적용한 것. 링크가 실제로 바뀐 곡은 기존대로 토큰이 증명한 출처(MANUAL/AUTO)를 따른다. 회귀 테스트 2건 추가(`user-quiz-registration.service.spec.ts`).
...
- ~~기존 AUTO 링크를 그대로 둔 채 다른 필드만 수정하는 경로가 막혀 있음~~ — **완전히 해결됨(2026-09-03, 코드 리뷰 6차)**: 수정 화면 프리필이 저장된 링크를 신뢰하고 다시 콘텐츠 검증을 하지 않게 되면서, "AUTO로 등록된 링크가 재검증에서 표기 차이로 실패"할 여지 자체가 사라졌다. 위 "검증 토큰이 진짜로 필요해진 이유" 항목 참고.
- **정답 후보 응답이 늦게 도착하면 사용자가 직접 입력한 정답을 덮어쓸 수 있음**: [`QuizBuilderPage.handleAutoFill`](../../../apps/web/src/pages/QuizBuilderPage.tsx)과 [`SongLinkAnswerModal`](../../../apps/web/src/components/SongLinkAnswerModal.tsx)의 `getAnswerCandidates()` 호출에는 링크 검증과 달리 순번 보호가 없다. 자동 채우기 후 정답 후보를 불러오는 동안 유저가 카드를 열어 직접 정답을 저장하면, 늦게 도착한 기본 후보가 방금 입력한 정답을 덮어쓸 수 있다(코드 리뷰 지적, 우선순위상 지금은 보류하기로 결정). 고칠 때는 함수형 `setSongs`로 적용 시점에 정답이 여전히 비어 있는지 다시 확인하거나, 링크 검증과 같은 요청 순번 방식을 정답 후보 조회에도 적용한다.
- **만료된 로컬 초안을 그대로 확인 완료 상태로 복구함**: 검증 토큰은 1시간 뒤 만료되지만, `localStorage` 초안([`quizDraft.ts`](../../../apps/web/src/utils/quizDraft.ts))에는 `status: 'valid'`와 토큰을 만료 여부와 무관하게 그대로 저장해뒀다가 그대로 복구한다(코드 리뷰 지적, 우선순위상 지금은 보류하기로 결정). 하루 뒤 초안을 열어도 화면은 전부 ✅로 보이고 등록 버튼도 활성화되지만, 실제 제출 시 서버가 만료된 토큰을 거부해서 그제서야 곡마다 다시 확인해야 하는 걸 알게 된다. 고칠 때는 초안에 토큰 발급/만료 시각을 함께 저장해두고, 불러올 때 만료된 `valid`나 중단된 `checking` 상태를 `unverified`로 되돌린다(토큰의 `exp`는 UX 판단에만 쓰고, 최종 신뢰는 항상 서버 재검증이 담당).
...
- 2026-09-03: 코드 리뷰 6차 - `GET /quizzes/:quizId`가 곡 수만큼 유튜브 검증을 동시에 실행해 외부 요청이 무제한으로 몰릴 수 있다는 지적을 받고, "저장된 링크를 다시 스크래핑하지 않고 그대로 신뢰"하는 방향으로 설계를 변경(안전망을 통과 못 한 QuizSong 행은 삭제되므로, 존재하는 행 자체가 검증 통과의 증거라는 논리). 같은 라운드의 나머지 2건(정답 후보 응답 경쟁, 만료된 초안의 확인 완료 상태 복구)은 우선순위상 지금은 보류하고 "아직 안 고친 것"에 기록.
- 2026-09-03: 코드 리뷰 7차 - 6차 변경의 부작용으로, 수정 화면이 항상 발급하는 MANUAL 토큰 때문에 원래 AUTO로 등록됐던 곡이 설명만 고쳐도 수정 안전망에서 삭제될 수 있던 버그 수정. `updateQuiz`가 songId별 기존 videoId를 기억해뒀다가 제출값과 같으면("링크 안 건드림") 안전망 제목 매칭을 생략하도록 함.
- 2026-09-03: 여러 유저가 동시에 등록하면 유튜브 스크래핑 요청이 몰려 차단될 위험이 있어 `YOUTUBE_LINK_VERIFICATION_ENABLED`를 꺼서 유튜브 링크 검증(즉시 검증의 콘텐츠 대조, 자동 찾기)을 임시로 비활성화. 등록 신청을 큐로 순차 처리하는 방식으로 바뀌면 재활성화할 예정이나, 그 큐는 이번 범위에 포함하지 않음(별도 작업).
