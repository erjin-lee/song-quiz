...
- 상태 관리는 별도 라이브러리 없이 React Context(`src/context/SessionContext.tsx`)와 페이지별 로컬 state로 처리한다. 방(room) 입장 직후의 `room`/`userId`는 `react-router-dom`의 location state로 다음 페이지에 전달한다(새로고침 시 유지되지 않음 — 알려진 한계).
