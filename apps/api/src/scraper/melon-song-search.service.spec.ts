...
describe('MelonSongSearchService', () => {
  ...
  const cacheServiceMock = { getStrict: jest.fn(), setStrict: jest.fn() };
  ...
  describe('search', () => {
    it('검색 결과에 "곡명 - 가수명" 라벨을 붙여 반환하고 melonSongId별로 캐시해둔다', async () => {
      ...
      expect(cacheServiceMock.setStrict).toHaveBeenCalledWith(
  ...
  describe('registerFromMelonSongId', () => {
    it('이미 있는 곡이면 새로 만들지 않고 그대로 반환한다', async () => {
      ...
      expect(cacheServiceMock.getStrict).not.toHaveBeenCalled();
    });

    it('캐시된 검색 결과가 없으면 재검색을 요청한다', async () => {
      songRepositoryMock.findOne.mockResolvedValue(null);
      cacheServiceMock.getStrict.mockResolvedValue(undefined);
    ...
    it('클라이언트가 곡명/아티스트명을 별도로 보내도 무시하고 캐시된 검색 결과만 신뢰한다', async () => {
      songRepositoryMock.findOne.mockResolvedValue(null);
      cacheServiceMock.getStrict.mockResolvedValue(cachedSearchResult);
    ...
    it('동시 등록으로 곡 저장이 unique 충돌하면 재조회한 값을 반환한다(락 조회로 최신 커밋본을 읽는다)', async () => {
      songRepositoryMock.findOne.mockResolvedValue(null);
      cacheServiceMock.getStrict.mockResolvedValue(cachedSearchResult);
      managerMock.findOne
      ...
      managerMock.save.mockImplementation((entity: unknown) => {
        if (entity === Song) return Promise.reject(duplicateKeyError());
      ...
      expect(managerMock.findOne).toHaveBeenLastCalledWith(
        Song,
        expect.objectContaining({ lock: { mode: 'pessimistic_read' } }),
      );
    });

    it('unique 충돌이 아닌 다른 에러는 재조회하지 않고 그대로 던진다', async () => {
      songRepositoryMock.findOne.mockResolvedValue(null);
      cacheServiceMock.getStrict.mockResolvedValue(cachedSearchResult);
      managerMock.findOne
        .mockResolvedValueOnce({ atstId: 'a1' }) // artist lookup
        .mockResolvedValueOnce({ albmId: 'al1' }); // album lookup
      managerMock.save.mockImplementation((entity: unknown) => {
        if (entity === Song) return Promise.reject(new Error('DB 연결 끊김'));
        return Promise.resolve({});
      });
      ...
      expect(managerMock.findOne).toHaveBeenCalledTimes(2); // artist/album만, song 재조회 없음
    });
  });
});
